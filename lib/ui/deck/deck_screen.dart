...
class _TopBar extends StatelessWidget {
  ...
  List<DeckSummary> _editableDecks(WsLayoutSource live) =>
      live.decks.where((deck) => deck.id != 'launcher').toList(growable: false);
  ...
  Future<void> _pickDeck(BuildContext context) async {
    final live = session;
    if (live == null) return;
    final decks = _editableDecks(live);
      ...
      _StripButton(
        ...
        onTap: () {
          ...
          final fixedDeck = session == null
              ? null
              : _editableDecks(session!).firstOrNull;
          if (fixedDeck != null) {
