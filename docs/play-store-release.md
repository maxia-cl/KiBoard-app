...
5. Complete Data safety consistently with the policy: KiBoard has no accounts, ads, analytics, or
   KiBoard cloud backend. Pairing data stays on device. Dictation invokes the Android speech
   recognition service and sends only the resulting text to the paired PC over the local network.
...
> encrypted connection—no KiBoard account, ads, analytics, or cloud service. The open-source
> Windows companion is required and is available from the KiBoard GitHub repository.
